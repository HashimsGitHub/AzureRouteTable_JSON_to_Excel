...
# --- Page Configuration ---
st.set_page_config(
    page_title="Azure Route Table Converter",
    page_icon="🎈",
    layout="wide"
)
...
st.markdown("""
    <div style="display:flex; align-items:center; gap:15px; background-color:#0078D4; padding:15px; border-radius:10px; color:white;">
        ...
        <h1 style="margin:0;">Azure Route Table - JSON to Excel Converter</h1>
    </div>
    <p style="margin-top:10px; font-size:16px; color:#333;">
        Upload your Azure Route Table (JSON export) and generate a clean, formatted Excel report 🚀
    </p>
...
def format_location(loc: str) -> str:
    ...
    region_map = {
        ...
        "southindia": "South India", "centralindia": "Central India", "westindia": "West India",
        "chinanorth": "China North", "chinanorth2": "China North 2",
        "chinaeast": "China East", "chinaeast2": "China East 2",
        ...
        "ukwest": "UK West", "uksouth": "UK South",
        ...
        "mexicocentral": "Mexico Central", "chilecentral": "Chile Central",
        "uaecentral": "UAE Central", "uaenorth": "UAE North", "qatarcentral": "Qatar Central",
        "southafricanorth": "South Africa North", "southafricawest": "South Africa West",
        "israelcentral": "Israel Central",
        "usgovvirginia": "US Gov Virginia", "usgovarizona": "US Gov Arizona",
        "usgoviowa": "US Gov Iowa", "usgovtexas": "US Gov Texas",
        "usdodeast": "US DoD East", "usdodcentral": "US DoD Central",
        "global": "Global", "centraluseuap": "Central US EUAP", "eastus2euap": "East US 2 EUAP"
    }
...
# --- File Upload Section ---
uploaded_file = st.file_uploader("📂 Upload your Azure Route Table JSON file", type="json")
...
if uploaded_file is not None:
    ...
    for i in range(1, len(df_routes.columns) + 1):
    ...
    for i in range(1, len(df_sub.columns) + 1):
        cell = ws[f"{get_column_letter(i)}{ws.max_row}"]
        cell.font = bold
        cell.fill = hdr_fill
    ...
    # --- Streamlit Display ---
    st.markdown("### 📘 Route Table Metadata")
    st.table(pd.DataFrame(list(metadata.items()), columns=["Field", "Value"]))

    st.markdown("### 🚦 Route Table Routes")
    st.dataframe(df_routes, use_container_width=True)

    st.markdown("### 🌐 Route Table Subnets")
